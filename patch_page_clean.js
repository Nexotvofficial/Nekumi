...
// 4. Update map to use visibleCount
...
// 5. Inject Pagination Button SAFELY
const loadMoreBlock = `
                </div>
...
//               )}
//             </section>
const sectionEndRegex = /<\/div>\s*\)\s*\}\s*<\/section>/;
if(!pageContent.includes('Cargar más manhwas')) {
    // Instead of replacing the whole block, just replace `</div>` with `</div> {loadMoreBlock} ` 
    // Wait, the ternary structure is:
    // {filteredManhwas.length === 0 ? ( <div...></div> ) : ( <div class="grid">...</div> )}
    // If we want to add pagination button, we MUST use a Fragment inside the false branch of the ternary.
    
    // Step 5a: Add `<>` right after `) : (`
    const branchStartRegex = /\) :\s*\(\s*<div className="grid grid-cols-2/;
    pageContent = pageContent.replace(branchStartRegex, `) : (\n                <>\n                <div className="grid grid-cols-2`);
    
    // Step 5b: Add the button and `</>` right before `)\n              }`
    // We can search for the end of the map block
    const mapEndRegex = /<\/div>\s*\)\s*\}/;
    pageContent = pageContent.replace(mapEndRegex, `</div>\n                  {visibleCount < filteredManhwas.length && (\n                    <div className="flex justify-center mt-10">\n                      <button onClick={() => setVisibleCount(v => v + 18)} className="bg-[#1c1c24] hover:bg-[#2a2a35] border border-white/5 text-white font-medium py-3 px-8 rounded-full transition-all active:scale-95 flex items-center gap-2"><span>Cargar más manhwas</span></button>\n                    </div>\n                  )}\n                </>\n              )}`);
}
